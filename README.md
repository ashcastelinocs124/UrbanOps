...
- **Live Plan Execution** — Executing a plan modifies real-time map data: affected roads close (0 mph, dark red), alternate routes speed up (green), transit halts near the incident, status changes to "RESPONDING"
- **Affected Region Visualization** — Plan preview shows impact zones, road closures with "CLOSED" labels, reroute paths with "REROUTE" labels, and dispatch unit markers on the map
- **Plan Status Tracking** — "PLAN UNDERWAY" banner during execution, "PLAN COMPLETED — INCIDENT RESOLVED" on completion, automatic removal from active incidents
...
- **Incident Panel** — Sorted by severity with PLAN button, clickable to filter AI recommendations, with status badges
...
| **Stream Processor** | FastAPI | 8011 | Subscribes to Redis, maintains city state, serves WebSocket + REST, executes plans |
| **LLM Analyst** | FastAPI + OpenAI | 8012 | Analyzes incidents via GPT-4o, publishes recommendations + generates detailed plans |
| **Frontend** | Next.js + Mapbox GL | 3001 | Palantir-style operations dashboard with login |
...
```bash
git clone https://github.com/OpenAgents-Illinois/UrbanOps.git
cd UrbanOps
cp .env.example .env
```
...
NEXT_PUBLIC_MAPBOX_TOKEN=pk.your_token_here
NEXT_PUBLIC_WS_URL=ws://localhost:8011/ws
NEXT_PUBLIC_ANALYST_URL=http://localhost:8012
NEXT_PUBLIC_PROCESSOR_URL=http://localhost:8011
```
...
cd services/simulator && REDIS_URL=redis://localhost:6379 PYTHONPATH=../services uvicorn main:app --port 8010
cd services/processor && REDIS_URL=redis://localhost:6379 PYTHONPATH=../services uvicorn main:app --port 8011
cd services/analyst && REDIS_URL=redis://localhost:6379 OPENAI_API_KEY=your_key PYTHONPATH=../services uvicorn main:app --port 8012
...
Login with any access code to enter the operations center.
...
| `/api/execute-plan` | POST | Processor | Execute plan — modifies live traffic, transit, and incident data |
